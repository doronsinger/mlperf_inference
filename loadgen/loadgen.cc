...
class QueryMetadata {
 ...
 public:
  SampleCompleteDelagate* sample_complete;
  uint64_t sequence_id;
...
void QuerySamplesComplete(QuerySampleResponse* responses,
  ...
  for (QuerySampleResponse* response = responses; response < end; response++) {
    ...
    query->sample_complete->Notify(query, sample, response, timestamp);
  }
}

// TODO: Non-detailed logs.
template <bool kNotifyImmediately, bool kCopyData>
struct SampleCompleteDetailed : public SampleCompleteDelagate {
  ...
  void Notify(QueryMetadata* query,
    ...
    std::vector<uint8_t> sample_data_copy;
    if (kCopyData) {
      auto* src_begin = reinterpret_cast<uint8_t*>(response->data);
      auto* src_end = src_begin + response->size;
      sample_data_copy = std::vector<uint8_t>(src_begin, src_end);
    }

    if (kNotifyImmediately) {
      // Notify immediately is useful for the blocking modes that limit
      // queue depth.
      Log(logger,
      ...
      // Delayed notification is useful for non-blocking modes that don't
      // have a hard limit on queue depth.
      Log(logger,
          [query = query,
           sample = sample,
           origin_time = logger->origin_time(),
...
void RunVerificationMode(SystemUnderTest* sut, QuerySampleLibrary* qsl,
  ...
  SampleCompleteDetailed<true, true> sample_complete_delegate(logger);
  ...
  for (auto &loadable_set : loadable_sets) {
    ...
    for (auto &query : queries) {
      ...
      issued_query_info.WaitForAllSamplesCompleted();
      ...
      issued_query_info.sample_complete = &sample_complete_delegate;
  ...
  // Compute percentile.
  std::vector<std::chrono::nanoseconds> latencies = logger->GetLatencies();
