...
Logger::~Logger() {
  {
    std::unique_lock<std::mutex> lock(tls_loggers_mutex_);
    keep_io_thread_alive_ = false;
    tls_loggers_cv_.notify_all();
  }
  io_thread_.join();
...
void Logger::RegisterTlsLogger(TlsLogger* tls_logger) {
  if (tls_loggers_.size() >= max_threads_to_log_) {
    std::cerr << "Warning: More TLS loggers registerd than can"
                 "be active simultaneously.\n";
  }
  std::unique_lock<std::mutex> lock(tls_loggers_mutex_);
  tls_loggers_.insert(tls_logger);
}
